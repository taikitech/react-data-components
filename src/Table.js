...
function buildSortProps(col, sortBy, onSort) {
  var order, nextOrder, sortEvent;
  if(!col.hasOwnProperty('sort') && !col.sort) {
    order = sortBy.prop === col.prop ? sortBy.order : 'none';
    nextOrder = order === 'ascending' ? 'descending' : 'ascending';
    sortEvent = onSort.bind(null, { prop: col.prop, order: nextOrder }, col.type);
  } else {
    order = '';
    nextOrder = '';
  }
  ...
  return {
    'onClick': sortEvent ? sortEvent : null,
